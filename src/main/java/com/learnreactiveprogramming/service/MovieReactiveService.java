...
public class MovieReactiveService {
    ...
    public static Retry retrySpec() {
        ...
        return Retry.fixedDelay(3, Duration.ofMillis(1000))
                .filter((ex) -> ex instanceof MovieException)  // retry only for MovieException
                .onRetryExhaustedThrow(((retryBackoffSpec, retrySignal) -> Exceptions.propagate(retrySignal.failure())));  // Return Actual Exception
    }
    ...
    public Flux<Movie> getAllMovies_repeat() {
        ...
        var movies = movieInfoFlux
                ...
                .retryWhen(retrySpec())
               // .repeat();  // repeat indefinitely. gets invoked after onCompletion() event . Works as long a sno exception is thrown
                .repeat(1);   // repeat just once
    ...
    public Mono<Movie> getMovieById_withRevenue(long movieId) {
        ...
        // Note: revenueService.getRevenue(movieId) in below code is a blocking call and we will wrap it to make it a non-blocking  Mono
        var revenueMono = Mono.fromCallable(() -> revenueService.getRevenue(movieId))
                .subscribeOn(Schedulers.boundedElastic());    // invoke thread pool boundedElastic

        return  movieInfoMono.zipWith(reviewList, (movieInfo, reviews) -> new Movie(movieInfo, reviews)) // first zip using Mono<MovieInfo> and Mono<List<Review>>
                .zipWith(revenueMono, ((movie, revenue) -> {     // second zipWith using Mono<Movie> output from 1st zip and Mono<Revenue>
